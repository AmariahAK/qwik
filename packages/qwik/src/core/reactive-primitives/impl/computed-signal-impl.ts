...
import { assertFalse, assertTrue } from '../../shared/error/assert';
...
export class Job<T> implements ComputeCtx<T> {
  ...
  declare $track$: Tracker | undefined;
  declare $cleanups$: Parameters<ComputeCtx<T>['cleanup']>[0][] | undefined;
  ...
  implements BackRef
{
  ...
  constructor(
    ...
    if (initial !== undefined) {
      this.$untrackedValue$ = typeof initial === 'function' ? (initial as () => T)() : initial;
    }
  ...
  set untrackedPending(value: boolean) {
    if (value !== !!this.$untrackedPending$) {
      ...
      DEBUG && log('Set untrackedPending', value);
   ...
   *
   * Accessing .error will trigger computation if needed, since it's often used like `signal.error ?
   * <Failed /> : signal.value`.
   */
  ...
  get untrackedError() {
    // reading `.error` means someone wants the settled result, so trigger the computation
    this.$computeIfNeeded$();
  ...
  $computeIfNeeded$(): void {
    ...
    if (isPromise(result)) {
      ...
      let first: Promise<T> | undefined = result;
      const compute = () => {
        const r =
          first ?? (this.$invokeComputeFn$(this.$computeQrl$.resolved as Function, running) as T);
        first = undefined;
        return r;
      };
  ...
  $computeAsync$(): void {
    ...
    const running = new Job<T>(this, this.$info$, infoVersion);
    this.$current$ = running;
    if (this.$jobs$) {
  ...
  async $runComputation$(running: Job<T>): Promise<void> {
  ...
  $requestCleanups$(job: Job<T>, reason?: any) {
